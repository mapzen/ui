...
var MapzenGeolocator = (function () {
  ...
  var MapzenGeolocator = function (options) {

    console.log('MapzenGeolocator options:', options);
    ...
    if (!(this instanceof MapzenGeolocator)) return new MapzenGeolocator(options)

    // If iframed and MapzenGeolocator isn't explicitly activated, exit & do nothing.
    if (window.self !== window.top && options !== true) {
      return false
    }

    // If MapzenGeolocator turned off in options, exit & do nothing.
    if (options === false) {
      return false
    }
...
module.exports = MapzenGeolocator;
